...
import org.reactome.server.interactors.tuple.model.ColumnDefinition;
            ...
            while ((customInteraction = beanReader.read(CustomInteraction.class, header)) != null) {
                ...
                if (mandatoryMessages.size() == 0) {
                    ...
                    if (customResource.checkForDuplicates(customInteraction)) {
                        warningResponses.add(Response.getMessage(Response.DUPLICATE_AB, beanReader.getLineNumber(), customInteraction.getInteractorIdA(), customInteraction.getInteractorIdB()));
                    } else {
                        customResource.add(customInteraction);
                    }

                } else {
                    errorResponses.add(Response.getMessage(Response.MISSING_MANDATORY_FIELDS, beanReader.getLineNumber(), mandatoryMessages));
                }
            }
