...
public class TupleParser extends CommonParser {
    ...
    private void analyseHeaderColumns(List<String> data) {
        ...
        if (hasHeaderLine(headerLine)) {
            ...
            warningResponses.add(Response.getMessage(Response.NO_HEADER_WARNING));
            ...
            if (values.length > 0) {
                if (values.length == thresholdColumn) {
                    ...
                    if (customResource.checkForDuplicates(customInteraction)) {
                        warningResponses.add(Response.getMessage(Response.DUPLICATE_AB, (i + 1), customInteraction.getInteractorIdA(), customInteraction.getInteractorIdB()));
                    } else {
                        customResource.add(customInteraction);
                    }
                } else {
                    errorResponses.add(Response.getMessage(Response.COLUMN_MISMATCH, i + 1, thresholdColumn, values.length));
                }
            }
        }
    }
